...
export const TechDetailsSection: React.FC<TechDetailsSectionProps> = ({ title, items, colorTitle }) => {
    return (
        <section className="mb-10 pt-6">
            ...
            <dl className="space-y-8">
                {items.map((item) => (
                    <div key={item.id} id={item.id} className="bg-white p-6 rounded-lg shadow-md scroll-mt-20">
                        {item.icon && (
